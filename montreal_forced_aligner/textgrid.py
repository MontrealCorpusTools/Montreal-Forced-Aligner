...
from typing import TYPE_CHECKING, Optional, Dict, List, Tuple, Set, Union
if TYPE_CHECKING:
    from .dictionary import MappingType, PunctuationType, ReversedMappingType, DictionaryEntryType, \
        IpaType, DictionaryType, MultiSpeakerMappingType, WordsType
    from .multiprocessing.alignment import CtmType
    from .corpus import AlignableCorpus, OneToManyMappingType, OneToOneMappingType
    from .config import AlignConfig
    from  .corpus.classes import File, Speaker, DictionaryData
...
def split_clitics(item: str, words_mapping: MappingType, clitic_set: Set[str],
                  clitic_markers: PunctuationType, compound_markers: PunctuationType) -> List[str]:
...
def _lookup(item: str, words_mapping: MappingType, punctuation: PunctuationType, clitic_set: Set[str],
            clitic_markers: PunctuationType, compound_markers: PunctuationType) -> List[str]:
...
def to_int(item: str, words_mapping: MappingType, punctuation: PunctuationType, clitic_set: Set[str],
           clitic_markers: PunctuationType, compound_markers: PunctuationType, oov_int: int) -> List[int]:
    """
...
def parse_from_word(ctm_labels: List[CtmInterval], text: List[str], dictionary_data: DictionaryData) -> List[CtmInterval]:
    cur_ind = 0
    actual_labels = []
    utterance = None
...
def parse_from_word_no_cleanup(ctm_labels: List[CtmInterval], reversed_word_mapping: ReversedMappingType) -> List[CtmInterval]:
...
def parse_from_phone(ctm_labels: List[CtmInterval], reversed_phone_mapping: ReversedMappingType, positions: List[str]) -> List[CtmInterval]:
    for ctm_interval in ctm_labels:
...
def map_to_original_pronunciation(phones: CtmType,
                                  subpronunciations: List[DictionaryEntryType], strip_diacritics: IpaType) -> CtmType:
        ...
        for pi in p['original_pronunciation']:
            if pi == phones[mapping_ind].label:
                ...
                if modded_phone == new_p:
                    ...
                    if modded_phone == new_p:
                        new_phones.append(CtmInterval(phones[mapping_ind].begin, phones[mapping_ind + 1].end, new_p, phones[mapping_ind].utterance))
                        mapping_ind += 1
            mapping_ind += 1
    return new_phones
...
def ctms_to_textgrids_non_mp(aligner:AlignerType) -> None:
    ...
    def process_current_word_labels():
        ...
        if aligner.align_config.cleanup_textgrids:
            ...
            actual_labels = parse_from_word_no_cleanup(current_labels, speaker.dictionary_data.reversed_word_mapping)
        cur_utt.word_labels = actual_labels

    def process_current_phone_labels():
        speaker = cur_utt.speaker

        cur_utt.phone_labels = parse_from_phone(current_labels,
    ...
    for j in aligner.corpus.jobs:
        ...
        processed_files = set()
        for file_name, file in j.job_files().items():
...
def generate_tiers(file: File,
                   cleanup_textgrids: Optional[bool]=True) -> Dict[Speaker, Dict[str, CtmType]]:
    output = {}
    ...
    for u in file.utterances.values():
        ...
        if dictionary_data.multilingual_ipa and cleanup_textgrids:
            ...
            for interval in u.word_labels:
                ...
                word = interval.label
                subwords = _lookup(word, dictionary_data.words_mapping, dictionary_data.punctuation, dictionary_data.clitic_set,
                                   dictionary_data.clitic_markers, dictionary_data.compound_markers)
                subwords = [x if x in dictionary_data.words_mapping else dictionary_data.oov_code for x in subwords]
                ...
                phones.extend(map_to_original_pronunciation(cur_phones, subprons,
                                                            dictionary_data.strip_diacritics))
                if not word:
                    continue
            ...
            for interval in u.word_labels:
                words.append(interval)
            for j, interval in enumerate(u.phone_labels):
...
def export_textgrid(file: File, output_path: str, speaker_data: Dict[Speaker, Dict[str, CtmType]],
                 frame_shift: int,
                    first_file_write: Optional[bool]=True) -> None:
    if frame_shift > 1:
...
def ctm_to_textgrid(file: File,
                    aligner: AlignerType,
                    first_file_write=True) -> None:
    ...
    export_textgrid(file, output_path, data,
                     aligner.align_config.feature_config.frame_shift, first_file_write)
