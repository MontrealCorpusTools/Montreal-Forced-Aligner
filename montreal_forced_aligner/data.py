...
class DatabaseImportData:
    ...
    word_mapping: typing.Dict[
        typing.Union[typing.Tuple[int, int], str], typing.Dict[str, typing.Any]
    ] = dataclassy.factory(dict)
    ...
    def add_objects(self, other_import: DatabaseImportData) -> None:
        ...
        for key, d in other_import.word_mapping.items():
            if key not in self.word_mapping:
                self.word_mapping[key] = {"count": 0}
            self.word_mapping[key]["count"] += d["count"]
...
class WorkflowType(enum.Enum):
    ...
    reference = 0  #: Load alignments from reference directory
    alignment = 1  #: Align using corpus texts, acoustic model, and pronunciation dictionary
    transcription = (
        2  #: Transcribe using acoustic model, pronunciation dictionary, and language model
    )
    phone_transcription = 3  #: Transcribe using acoustic model and phone-based language model
    per_speaker_transcription = 4  #: Transcribe using acoustic model, pronunciation dictionary, and per-speaker language model generated by corpus texts
    speaker_diarization = 5  #: Diarize speakers
    online_alignment = 6  #: Online alignment
