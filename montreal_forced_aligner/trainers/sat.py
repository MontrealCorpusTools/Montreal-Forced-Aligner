    ...
    def align(self, subset: Optional[int]=None):
        ...
        if not os.path.exists(done_path):
            ...
            if subset is None:
                ...
                align_data_directory = self.corpus.subset_directory(subset)
            try:
                ...
                if align_data_directory == self.data_directory and os.path.exists(os.path.join(self.train_directory, 'trans.0')):
                    for i in range(self.corpus.num_jobs):
                        shutil.copy(os.path.join(self.train_directory, f'trans.{i}'),
                                    os.path.join(self.align_directory, f'trans.{i}'))
                ...
                if not os.path.exists(os.path.join(self.align_directory, 'trans.0.ark')):
                    calc_fmllr(self)
        ...
        try:
            ...
            if os.path.exists(os.path.join(align_directory, 'trans.0.ark')):
                for j in self.corpus.jobs:
                    for dict_name, path in j.construct_path_dictionary(align_directory, 'trans', 'ark').items():
                        shutil.copy(path,
                                    path.replace(align_directory, self.train_directory))
            else:
