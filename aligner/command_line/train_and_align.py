...
def align_corpus_no_dict(args):
    ...
    try:
        ...
        if args.config_path:
            ...
            train_config, align_config = load_basic_train()
        a = TrainableAligner(corpus, dictionary, train_config, align_config, args.output_directory,
                             temp_directory=data_directory, num_jobs=args.num_jobs, debug=args.debug,
                             skip_input=getattr(args, 'quiet', False))
        a.verbose = args.verbose
        a.train()
        a.export_textgrids()
...
if __name__ == '__main__':  # pragma: no cover
    ...
    parser.add_argument('dictionary_path', help='Full path to the pronunciation dictionary to use', nargs='?',
                        default='')
